...
**HelpeRP** — десктопный оверлей для RP-игроков (GTA 5 RP, SAMP, CRMP): база знаний по фракциям, меры и наказания, шаблоны отыгровок, умный поиск и ИИ-генерация `/me`, `/do`.
...
### База знаний
- Статьи, протоколы и словарь по **11+ фракциям** (МВД, СК, СМП, МЧС, ФСБ, Армия…)
- **RAG-поиск** (BM25 + ключевые слова) — находит по смыслу, не только по точному совпадению
- **Wikipedia / Викисловарь** при пустом локальном результате
- Раздел **«Энциклопедия»** с кэшем просмотренных статей
...
### ИИ-отыгровки
- OpenAI-совместимые API: **OpenAI, DeepSeek, Groq, OpenRouter, Mistral…**
...
- **7 тем оформления** + свой акцент (#hex)
- Плавные анимации, toast-уведомления
- Автообновления с проверкой подписи manifest

### Безопасность (v2)
- Лицензии **Ed25519** — приватный ключ только у продавца, не в exe
...
- **Поддержка:** support@example.com *(замените в `core/version.py` перед продажей)*
