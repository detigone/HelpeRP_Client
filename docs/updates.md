...
1. Укажите `UPDATE_MANIFEST_URL` в `core/version.py` (уже настроено для GitHub Pages)
2. Push в `main` — сайт и manifest деплоятся автоматически
3. Релиз exe: `py tools/release.py --bump patch` → загрузить zip в GitHub Releases

**GitHub Pages (бесплатно, без домена):**

```
https://yeolka-lm.github.io/HelpeRP_Client/
https://yeolka-lm.github.io/HelpeRP_Client/updates/manifest.json
```

Включите: репозиторий → Settings → Pages → Source: **GitHub Actions**
